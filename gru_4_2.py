...
# additionally this model has convolutional layers on top which are supposed to be better at extracting features and patterns from the data
from keras.models import Sequential
from keras.layers import Dense, Dropout, GRU, Conv1D, MaxPooling1D

gru = Sequential()
gru.add(Conv1D(6, 3, activation='relu', input_shape=(1500,1)))
gru.add(MaxPooling1D(pool_size=5))
gru.add(Dropout(0.3))
gru.add(Conv1D(6, 3, activation='relu'))
gru.add(MaxPooling1D(pool_size=3))
...
csvlogger = CSVLogger(
    filename= "./gru4_2/training_csv.log",
    separator = ",",
    append = False
)
